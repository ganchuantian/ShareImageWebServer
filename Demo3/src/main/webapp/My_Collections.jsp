...
<html>
  ...
  <body style="background-image: url(imgs/stock-photo-278825773.jpg);">
  	<%@ include file="top.jsp"%>
  	<center>
     	<div class="Pics" style="margin-left:20px;width:890px;">
			<div id="title" style="width:850px;">
				<div style="margin-top: 5px;">所有收藏</div>
			</div><br />
			<div class="pics-box" id="pics-box">	
				...
				<!--<c:forEach items="${requestScope.rs}" var="keyword">
					...
					<div class="Works_Box" onclick="detail('${keyword.p_id}')">
						...
						<div class="delete_btn" onclick="deleteCollection(${keyword.p_id})">取消收藏</div>
					</div>
            ...
            $.getJSON("HP_GetNextPics", {
				...
				index: index,
					...
					$.each(data, function(index1) {
						...
						html += '		<img src="http://localhost:9080/PicBase/'+data[index1].P_path+'">';
						html += '	</div>';
						...
						html += '	</div>';
						html += '<div class="delete_btn" onclick="deleteCollection('+data[index1].P_id+')">取消收藏</div>';
						html += '</div>';
						$("#ct").append(html);
						i = index1;
					});
  ...
  function deleteCollection(p_id){
